...
contract('Protocol test', async (accounts) => {
    ...
    it('check single transfer', async () => {
        ...
        const privateKey = bobsDepositWitness.getR().sub(witness.getR()).sub(witness_change.getR()).umod(parameters.getGroup().order);
        ...
        const encryptedBytes_change = aesCtrEncryptor_change.encrypt(dataToEncrypt_change);

        const prover = new RangeProofProver();
        const rangeVerifier = new RangeProofVerifier();

        const proof = prover.generateProof(parameters, witness.getCommitment(), witness);
        const consCheck = rangeVerifier.verify(parameters, witness.getCommitment(), proof);
        assert(consCheck, "failed internal test")
        const proof_change = prover.generateProof(parameters, witness_change.getCommitment(), witness_change);
        const consCheck_change = rangeVerifier.verify(parameters, witness_change.getCommitment(), proof_change);
        assert(consCheck_change, "failed internal test")
        
        const _scalars = [from_X, from_Y, to_0_X, to_0_Y, to_1_X, to_1_Y, index_0, index_1, s_0, e_0, s_1, e_1, assetID];

        const coords = [];
        coords.push(witness.getCommitment().getX()) //commitment itself
        coords.push(witness.getCommitment().getY())
        ...
        coords.push(witness_change.getCommitment().getX()) //commitment itself
        coords.push(witness_change.getCommitment().getY())
        coords.push(proof_change.getaI().getX())
        coords.push(proof_change.getaI().getY())
        coords.push(proof_change.getS().getX())
        coords.push(proof_change.getS().getY())
        const tCommits_change = proof_change.gettCommits();
        coords.push(tCommits_change.get(0).getX())
        coords.push(tCommits_change.get(0).getY())
        coords.push(tCommits_change.get(1).getX())
        coords.push(tCommits_change.get(1).getY())
        ...
        for (let i=0; i < proof_change.getProductProof().getL().length; i++) {
            const L = proof_change.getProductProof().getL()[i];
            const R = proof_change.getProductProof().getR()[i];
            ls_coords.push(L.getX())
            ls_coords.push(L.getY())
            rs_coords.push(R.getX())
            rs_coords.push(R.getY())
        ...
        const scalars = [proof.getTauX(), proof.getMu(), proof.getT(), proof.getProductProof().getA(), proof.getProductProof().getB(),
            proof_change.getTauX(), proof_change.getMu(), proof_change.getT(), proof_change.getProductProof().getA(), proof_change.getProductProof().getB()]
        
        const gasEstimate = await coinMixer.transfer.estimateGas(_scalars, ethUtil.bufferToHex(Buffer.from(encryptedBytes)), ethUtil.bufferToHex(Buffer.from(encryptedBytes_change)), coords, scalars, ls_coords, rs_coords, {from: operator})
        console.log("Transfer with " + M + " bits to prove is " + gasEstimate + " gas")
        const transferResult = await coinMixer.transfer(_scalars, ethUtil.bufferToHex(Buffer.from(encryptedBytes)), ethUtil.bufferToHex(Buffer.from(encryptedBytes_change)), coords, scalars, ls_coords, rs_coords, {from: operator})
        ...
        const restoredEncreyptedBytes = ethUtil.toBuffer(allTransferEventsResult[0].args._data);
